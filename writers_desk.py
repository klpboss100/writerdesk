...
- "suggestion"은 "original" 자리에 그대로 갖다 끼워넣기만 하면 되는 **완성된 대체 텍스트**여야 합니다.
  사람이 다시 손볼 필요가 전혀 없어야 합니다.
- "suggestion"은 절대 "지시문"이나 "설명문"이 되어서는 안 됩니다.
  - 절대 금지 표현: "~할 것", "~로 교체하여 ~줄일 것/분산할 것/통일할 것", "~로 표현할 것",
    "~로 바꿀 것", "~등으로 교체", "~등의 유의어로", "~사용을 유지", "~표현 유지",
    "삭제 권장", "수정 가능", "수정 요망", "그대로 두어도 됨", "~수정하세요", "~검토 필요"
  - 위와 같이 "방법을 알려주는 문장"이 아니라, **그 방법을 적용한 결과물 자체**를 적으세요.
- 절대 비워두지 마세요.
- 반복단어(repeat_words) 타입의 경우:
  - "original"은 실제로 반복되는 단어/구절 중 한 곳의 정확한 표현이어야 합니다.
  - "suggestion"은 그 자리에 바로 바꿔 넣을 **구체적인 동의어 1~2개**여야 합니다
    (예: "원고에 등장하는 같은 표현을 ①②로 교체하라"는 식의 안내문 금지).
- 최소 1개, 가능하면 2개의 구체적인 대안을 제시하세요.
- 대안이 2개 이상일 경우 다음 형식으로 작성하세요: "① 대안A ② 대안B"
- 예시:
  - 올바른 예 (어색한 문장): "그의 눈빛이 그 마음을 대신했다"
  - 올바른 예 (반복단어, 복수안): "① 의심 선박 ② 정체불명의 선박"
  - 올바른 예 (반복단어, 복수안): "① 끊임없이 ② 쉼 없이"
  - 잘못된 예: "정체불명의 선박, 의심 선박, 침투 선박 등으로 교체하여 반복을 줄일 것"
  - 잘못된 예: "끊임없이, 쉼 없이, 이어, 연이어 등의 유의어로 교체하여 반복을 분산할 것"
...
{{
  "issues": [
    {{
      "original": "원고에서 정확히 찾을 수 있는 텍스트 (반복단어는 실제 등장하는 표현 그대로)",
      "suggestion": "original 자리에 바로 끼워넣을 완성된 대체 텍스트 1~2개 (① ② 형식 가능, 지시문/설명문/빈 값 절대 금지)",
...
def sanitize_issues(result):
    """suggestion이 비어있거나 지시문/설명문(모호한 안내)인 경우 원본 문장으로 대체 (빈 제안 방지)"""
    vague_markers = ["삭제 제안", "삭제 권장", "표현 유지", "수정 가능", "수정 요망",
                     "그대로 두", "수정하세요", "다듬어", "자연스럽게 수정",
                     "바꾸는 것이 좋습니다", "고려해", "검토해", "검토 필요",
                     "사용을 유지", "교체하여", "교체할 것", "교체하라",
                     "유의어로", "등으로 교체", "등으로 표현", "분산할 것",
                     "통일할 것", "줄일 것"]
    # "~할 것", "~할 것." 처럼 지시문 형태로 끝나는 경우도 차단
    vague_endings = re.compile(r"(할\s*것|것\.?)\s*$")

    for issue in result.get("issues", []):
        sugg = (issue.get("suggestion") or "").strip()
        is_vague = (not sugg
                    or any(m in sugg for m in vague_markers)
                    or vague_endings.search(sugg))
        if is_vague:
